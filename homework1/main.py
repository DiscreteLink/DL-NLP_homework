...
def trigram_model(uni_dic,bi_dic,tri_dic):
    begin = time.time()
...
for file_name in file_names:
    ...
    s = file_name + " 按字分割 1-Gram 信息熵为" + str(unigram_entropy)
    print(s)
    bigram_entropy, timing = bigram_model(uni_dic, bi_dic)
    s = file_name + " 按字分割 2-Gram 信息熵为" + str(bigram_entropy)
    print(s)
    trigram_entropy, timing = trigram_model(uni_dic, bi_dic, tri_dic)
    s = file_name + " 按字分割 3-Gram 信息熵为" + str(trigram_entropy)
    print(s)
    # 再进行词分割
    ...
    tri_dic = get_trigram_wf(words)
    unigram_entropy,timing = unigram_model(uni_dic)
    s = file_name + " 按词分割 1-Gram 信息熵为" + str(unigram_entropy)
    print(s)
    bigram_entropy, timing = bigram_model(uni_dic, bi_dic)
    s = file_name + " 按词分割 2-Gram 信息熵为" + str(bigram_entropy)
    print(s)
    trigram_entropy, timing = trigram_model(uni_dic, bi_dic, tri_dic)
    s = file_name + " 按词分割 3-Gram 信息熵为" + str(trigram_entropy)
    print(s)
